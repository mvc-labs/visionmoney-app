    ...
    <div class="home-specific">
      <div class="nft-container">
        ...
        <div class="nft-info" v-if="dataSource">
          <img
            :src="dataSource.content"
            alt=""
            class="icon-style"
            v-if="dataSource.contentType.includes('image')"
          />
          <div class="out" v-else>
            <div class="content-area">
              <div class="ordinal-content">
                {{ dataSource.contentBody }}
              </div>
              <div class="rats">{{ dataSource.outputValue }} sats</div>
            </div>
          </div>

          <div class="nft-name"># {{ dataSource.num }}</div>
          ...
          <div class="text">
            ...
            <div class="genesis">
              <div class="left gray">ID</div>
              <div class="right">{{ dataSource.id }}</div>
            </div>
            <div class="genesis">
              <div class="left gray">Address</div>
              <div class="right">{{ dataSource.address }}</div>
            </div>
            <div class="index">
              <div class="left gray">Output value</div>
              <div class="right">{{ dataSource.outputValue }}</div>
            </div>
            <div class="genesis">
              <div class="left gray">Preview</div>
              <div
                class="right"
                @click="topreview(dataSource.preview)"
                style="cursor: pointer; color: rgb(81 115 185 / 1)"
              >
                {{ dataSource.preview }}
              </div>
            </div>
            <div class="genesis">
              <div class="left gray">Content</div>
              <div
                class="right"
                @click="topreview(dataSource.content)"
                style="cursor: pointer; color: rgb(81 115 185 / 1)"
              >
                {{ dataSource.content }}
              </div>
            ...
            <div class="creator">
              <div class="left gray">Content Length</div>
              <div class="right">{{ dataSource.contentLength }}</div>
            </div>
            <div class="creator">
              <div class="left gray">Content Type</div>
              <div class="right">{{ dataSource.contentType }}</div>
            </div>
            <div class="creater-time">
              <div class="left gray">Timestamp</div>
              <div class="right">{{ $filters.dateTimeFormat(dataSource.timestamp) }}</div>
            </div>
            <div class="genesis">
              <div class="left gray">Genesis Transaction</div>
              <div class="right">{{ dataSource.genesisTransaction }}</div>
            </div>
          </div>
        </div>
      </div>
...
async function getDetail(params) {
  const res = await getBtcOrdinalDetail(params)
  if (res.code == 0) {
    return res.data.inscriptions[0]
  }
}
onMounted(async () => {
  const btcParams = {
    net: store.currentNet == 'Mainnet' ? 'livenet' : 'testnet',
    inscriptionId: route.params.inscriptionId,
  }
  dataSource.value = await getDetail(btcParams)
  checkDevice()
...
async function Transfer() {
  ...
  const privateKey = store.currentWalletPrivateKey
  const mvcAddress = store.currentWalletAddress
